...
export function Sidebar({
  ...
  return <aside className="w-64 bg-white border-r border-gray-100 flex flex-col h-full flex-shrink-0 z-20">
      <div className="p-8">
        ...
        <nav className="space-y-1">
          ...
          return <button key={item.id} onClick={() => onChangeView(item.id)} className={`
                ...
                `}>
                {isActive && <motion.div layoutId="activeTab" className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full" initial={{
              opacity: 0
            }} animate={{
              opacity: 1
            }} exit={{
              opacity: 0
                ...
                {item.badge ? <span className="ml-auto bg-blue-100 text-blue-600 py-0.5 px-2 rounded-full text-xs font-bold">
                    {item.badge}
                  </span> : null}
              </button>;
        })}
